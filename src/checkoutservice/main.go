...
func (cs *checkoutService) Check(ctx context.Context, req *healthpb.HealthCheckRequest) (*healthpb.HealthCheckResponse, error) {
    ...
    // liveness — shallow, just proves process is alive
    if req.Service == "" {
    ...
    dependencies := map[string]*grpc.ClientConn{
        "cartservice":    cs.cartSvcConn,
        "paymentservice": cs.paymentSvcConn,
        "shippingservice": cs.shippingSvcConn,
    }

    for name, conn := range dependencies {
        if conn.GetState() == connectivity.TransientFailure || 
           conn.GetState() == connectivity.Shutdown {
            log.Warnf("health check: %s is in state %s", name, conn.GetState())
            return &healthpb.HealthCheckResponse{
